...
def perspective_transform(img,src,dst,inv=0,prn=0,nm='perspective_transform'):
    orig=np.copy(img)
    ...
    #warped = cv2.rectangle(warped, rect_cord[1], rect_cord[3], color=(0, 0, 255),thickness=5)
    if prn==1:
        print_images(orig,warped,nm)
...
def fit_poly(warped,leftx,lefty,rightx,righty,prn=0,nm='fit_poly'):
    ...
    cv2.fillPoly(window_img,np.int64([pts_left]),color=(0,255,0))
    cv2.fillPoly(window_img,np.int64([pts_right]), color=(0, 255, 0))
    result=img_weighted(out_img,window_img)

    # plt.plot(leftx_plot,ploty,color='yellow')
    # plt.plot(rightx_plot, ploty, color='yellow')
...
src=np.float32([[190,718],[580,457],[705,457],[1140,719]])
dest=np.float32([[200,710],[200,50],[1100,50],[1100,710]])

for name in glob.glob('test_images/*.jpg'):
    lane_image=cv2.imread(name)
    undst=cal_undistort(lane_image,mtx,dist)
    sobx=sobel_threshold(undst, orient='x',kern=3, thresh=(30, 250))
    #soby=sobel_threshold(lane_image, orient='y',kern=3, thresh=(30, 250),prn=1)
    #sobm=sobel_threshold(lane_image, orient='m',kern=3, thresh=(100, 250),prn=1)
    #sobgr=sobel_threshold(lane_image, orient='gr',kern=3, thresh=(0.7, 1.1),prn=1)
    ...
    processed_img=img_weighted(lane_image,warped.astype(np.uint8))
    ...
    output_filepath='output_images/'+name[12:]
    cv2.imwrite(output_filepath,final_img)

    print_images(lane_image,final_img,nm='lanes')
    #perspective_transform(dist_img)
